...
1. Clone the repository
   ```bash
   git clone https://github.com/Juwww233/goods-trading-platform.git
   cd goods-trading-platform
   ```
...
2. Start the Tomcat server (backend service)
3. Run the frontend project (development mode):
   ```bash
   cd shopping-frontend/vue
   npm run dev
   ```
4. Access the platform: http://localhost:8080 (adjust port according to your Tomcat/frontend configuration)

## Directory Structure
```plaintext
goods-trading-platform/
├── shopping-frontend/vue/  # Frontend source code
│   ├── src/
...
├── shopping-backend/       # Backend source code
│   ├── src/
│   │   ├── controller      # Servlet request controllers
